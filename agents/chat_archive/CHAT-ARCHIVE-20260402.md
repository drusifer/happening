...
[<small>2026-04-02 00:30:00</small>] [**Oracle**]->[**all**] **docs*:

 DOCUMENTED ✅ Six lessons filed in LESSONS.md from today's session (Bob):
 1. Settings panel overflow — `Positioned` needs `bottom` anchor for bounded height; `Expanded+ListView` fills it.
 2. Sign-in screen as TimelineStrip compositor layer (SignInLayer) — strip always mounted so `windowService.collapse()` fires at startup and window sizing is correct pre-auth.
 3. `Future.wait` per-calendar fetches — single 404 poisons whole batch; fix with `.catchError` per item.
 4. Clear `selectedCalendarIds` on sign-out — calendar IDs are account-scoped, stale ones cause 404s on next login.
 5. Don't set loading auth state during OAuth — strip disappears and user loses exit control.
 6. OAuth `server.first` blocks forever on window-close — store `_pendingServer`, expose `cancelSignIn()`, show "tap to cancel" in strip.
